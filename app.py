    ...
    try:
        if IP in pkt:
            ...
            row = {
                ...
                'Total Length of Fwd Packets': length, 'Total Length of Bwd Packets': 0,
                'Fwd Packet Length Max': length, 'Fwd Packet Length Min': length,
                'Fwd Packet Length Mean': length, 
                'Flow Bytes/s': length * 10, 'Flow Packets/s': 1,
                'Flow IAT Mean': np.random.randint(1, 100), 
                'Fwd Header Length': 20, 
                'Min Packet Length': length, 'Max Packet Length': length, 
                'Packet Length Mean': length, 'Average Packet Size': length,
...
if app_mode == "Dashboard":
    ...
    with col1:
        ...
        **Project Overview:**
        ...
        It is trained on the **CICIDS-2017** dataset and uses the **XGBoost** algorithm for high accuracy.
    ...
    with col2:
        ...
        st.metric(label="Model Accuracy", value="99.8%", delta="High")
        st.metric(label="Supported Attacks", value="14 Types")
        ...
        try:
            ...
            if st.button("Start Scan"):
                with st.spinner("Scanning traffic flows..."):
                    ...
                    st.dataframe(df[['Destination Port', 'Flow Duration', 'Prediction']], use_container_width=True)
